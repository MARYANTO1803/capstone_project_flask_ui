...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(by=['Category','App']).agg({
                    'Reviews': 'sum',
                    'Rating' : 'mean'}).sort_values(by=['Reviews'], ascending=False).iloc[0:10].to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    'Category' : 'count'
        }).rename({'Category':'Total'}, axis=1).sort_values(by=['Reviews']).head()
    X = cat_order.index
    Y = cat_order.Total
    my_colors = 'rgbkymc'
